    ...
    <div className='grid md:grid-cols-3 grid-cols-1 gap-5'>
      {products.map((item, i) => (
        <div key={i} className='bg-slate-50 dark:bg-slate-900 p-4 rounded text-center'>
          ...
          <span className='text-slate-500 dark:text-slate-300 text-sm mb-1 block font-normal'>{item.price}</span>
          <span className='text-slate-600 dark:text-slate-300 text-sm mb-4 block'>{item.title}</span>
          <a
            href='#'
