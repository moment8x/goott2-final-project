...
export const appEmailSlice = createSlice({
  ...
  reducers: {
    ...
    setCheck: (state, action) => {
      const emailIndex = state.emails.findIndex((email) => email.id === action.payload && email);
      state.emails[emailIndex].is_checked = !state.emails[emailIndex].is_checked;
    },
    ...
    setFaveCheck: (state, action) => {
      const favIndex = state.emails.findIndex((email) => email.id === action.payload && email);
    ...
    toggleReadMail: (state, action) => {
      const readIndex = state.emails.findIndex((email) => email.id === action.payload && email);
