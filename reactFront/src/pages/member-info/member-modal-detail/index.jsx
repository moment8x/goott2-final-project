...
import { toggleMobileEmailSidebar, toggleEmailModal, setFilter, setSearch } from './store';
...
const CrmPage = () => {
  ...
  const { mobileEmailSidebar, emails, search, filter, singleModal } = useSelector((state) => state.email);
  ...
  const filteredEmails = emails
    ...
    .filter((email) => {
      if (filter === 'all') {
        ...
        return email.draft;
      } else if (filter === 'spam' || filter === 'social' || filter === 'promotions') {
        return email.isspam;
  ...
  return (
    <>
      ...
      <div className='flex md:space-x-5 app_height overflow-hidden relative rtl:space-x-reverse'>
        ...
        ${width < breakpoints.lg && mobileEmailSidebar ? 'left-0 ' : '-left-full '}
        `}
        >
            ...
            <SimpleBar className='h-full px-6 '>
              <ul className='list mt-6'>
                {topFilterLists?.map((item, i) => (
                  <Topfilter item={item} key={i} filter={filter} onClick={() => handleFilter(item.value)} />
                ))}
              </ul>
              ...
              <ul>
                {bottomFilterLists?.map((item, i) => (
                  <BottomFilter item={item} key={i} filter={filter} onClick={() => handleFilter(item.value)} />
                ))}
              </ul>
            </SimpleBar>
          </Card>
        ...
        <div className='flex-1 md:w-[calc(100%-320px)]'>
          <Card bodyClass='p-0  h-full relative' className='h-full'>
            <EmailHeader onChange={(e) => dispatch(setSearch(e.target.value))} emails={filteredEmails} />
                ...
                <ul className='divide-y divide-slate-100 dark:divide-slate-700 -mb-6 h-full'>
                  ...
                  {filteredEmails.length === 0 && (
                    <li className='mx-6 mt-6'>
                      <Badge label='No Result Found' className='bg-danger-500 text-white w-full block text-start' />
                    </li>
                  )}
                </ul>
              )}
