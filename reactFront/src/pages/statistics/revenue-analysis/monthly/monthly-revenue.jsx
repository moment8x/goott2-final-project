...
const MonthlyReport = ({ setData, data }) => {
  const [picker, setPicker] = useState();
  const [picker2, setPicker2] = useState();

  useEffect(() => {
    fetch('http://localhost:8081/admin/statistics/monthly?dateStart&dateEnd', { method: 'GET' })
      .then((res) => res.json())
      .then((res) => {
        console.log('res:', res);
        setData(res.monthlyList);
      })
      .catch((error) => {
