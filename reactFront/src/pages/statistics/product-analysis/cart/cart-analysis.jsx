...
const CartAnalysis = ({ setData, setCartQuantity, data }) => {
  const [picker, setPicker] = useState();
  const [picker2, setPicker2] = useState();

  useEffect(() => {
    fetch('http://localhost:8081/admin/statistics/cart?dateStart&dateEnd', { method: 'GET' })
      .then((res) => res.json())
      .then((res) => {
        console.log('res:', res);
        setData(res.cartInfoList);
        setCartQuantity(res.cartTop10List);
      })
      .catch((error) => {
  ...
  return (
    <div>
      <Card title='장바구니상품 순위 내역' noborder className='font-bold'>
