...
const CategoryRanking = ({ setData, setTotalSalesData, setSalesQuantityData, data }) => {
  const [picker, setPicker] = useState();
  const [picker2, setPicker2] = useState();

  useEffect(() => {
    fetch('http://localhost:8081/admin/statistics/categories?dateStart&dateEnd', { method: 'GET' })
      .then((res) => res.json())
      .then((res) => {
        console.log('res:', res);
        setData(res.categoryRankingList);
        setSalesQuantityData(res.salesQuantityList);
        setTotalSalesData(res.totalSalesList);
      })
      .catch((error) => {
  ...
  return (
    <div>
      <Card title='판매분류 순위' noborder className='font-bold'>
