...
const ReturnRanking = ({ setData, setReturnRateData, setRefundQuantityData, data }) => {
  const [picker, setPicker] = useState();
  const [picker2, setPicker2] = useState();

  useEffect(() => {
    fetch('http://localhost:8081/admin/statistics/returns?dateStart&dateEnd', { method: 'GET' })
      .then((res) => res.json())
      .then((res) => {
        console.log('res:', res);
        setData(res.returnRankingList);
        setRefundQuantityData(res.refundQuantityRanking);
        setReturnRateData(res.returnRateRanking);
      })
      .catch((error) => {
  ...
  return (
    <div>
      <Card title='취소/반품 순위' noborder className='font-bold'>
