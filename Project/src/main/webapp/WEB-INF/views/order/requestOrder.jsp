         ...
         let couponDiscountAmount = Number($('#discountAmount'+selectedIndex).text());
            if (discountMethod == "P") {
               ...
               for(let i = 0; i<couponIndex.length; i++) {
                  if(selectedIndex == couponIndex[i].i) {
                	  // 헤이
                     for(let j = 0; j < couponIndex[i].index.length; j++) {   
                     couponAmount += Math.round(((Number($('#productPrice'+couponIndex[i].index[j]).text()) * Number($('#productQty'+couponIndex[i].index[j]).text())) * (couponDiscountAmount/100)) / 10) * 10;
